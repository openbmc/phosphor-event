...
namespace {
    ...
    char eventsDir[] = "./eventsXXXXXX";
    char* fd = mkdtemp(eventsDir);
    int rm = remove(fd);
...
public:
    ...
    void CreateEventPath()
    {
        ...
        int resultAsp = 0;
        int resultSys = 0;
        asprintf(&cmd, "exec rm -r %s 2> /dev/null", fd);
        system(cmd);
        resultAsp = asprintf(&cmd, "exec mkdir  %s 2> /dev/null", fd);
        if(resultAsp == -1){
            throw std::bad_alloc();
        }
    ...
    void RemoveEventPath()
    {
        ...
        resultAsp = asprintf(&cmd, "exec rm -r %s 2> /dev/null", fd);
        if(resultAsp == -1){
            throw std::bad_alloc();
        }
        resultSys = system(cmd);
